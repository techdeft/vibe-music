...
async function load() {
  ...
  try {
    ...
    if (playTrackId && album.value?.tracks) {
      const trackToPlay = album.value.tracks.find(t => String(t.id) === String(playTrackId))
      if (trackToPlay) {
        player.playTrack(trackToPlay, album.value.tracks)
      }
    }
  } catch (e) {
    console.error(e)
...
.tracks-header-row {
  display: grid;
  grid-template-columns: 48px 1fr 100px 60px 110px;
  padding: 0 16px;
  margin-bottom: 6px;
}
